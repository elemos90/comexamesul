...
class GuideController extends Controller
{
    ...
    private function markdownToHtml(string $markdown): string
    {
        ...
        // Links [text](url) - use callback to apply url() for internal links
        $markdown = preg_replace_callback('/\[([^\]]+)\]\(([^\)]+)\)/', function ($matches) {
            $text = $matches[1];
            $href = $matches[2];

            // If it's an internal link starting with /, apply url()
            if (str_starts_with($href, '/')) {
                $href = url($href);
            }

            return '<a href="' . htmlspecialchars($href) . '" class="text-blue-600 hover:text-blue-800 underline">' . htmlspecialchars($text) . '</a>';
        }, $markdown);
