...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="/applications/export" class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 transition-colors flex items-center gap-2">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800"><?= number_format($generalStats['total_applications']) ?></p>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-yellow-600"><?= number_format($generalStats['pending_count']) ?></p>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600"><?= number_format($generalStats['approved_count']) ?></p>
                </div>
            ...
            <p class="text-xs text-gray-500 mt-2">Taxa: <?= number_format($generalStats['approval_rate'] ?? 0, 1) ?>%</p>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-purple-600"><?= number_format($generalStats['avg_review_hours'] ?? 0, 1) ?>h</p>
                </div>
            ...
            <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                    ...
                    <p class="text-sm text-red-700 mt-1">
                        ...
                        <a href="/applications" class="underline font-medium">Revisar agora →</a>
                    </p>
                </div>
            </div>
        </div>
                    ...
                    <?php foreach ($statusDistribution as $item): ?>
                        <div>
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="h-2 rounded-full transition-all" style="width: <?= $item['percentage'] ?>%; background-color: <?= $item['color'] ?>;"></div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    ?>
                        <div class="flex items-center gap-3">
                            <span class="text-xs font-medium text-gray-500 w-20"><?= date('d/m', strtotime($day['date'])) ?></span>
                            <div class="flex-1 bg-gray-200 rounded-full h-6 relative">
                                <div class="bg-primary-500 h-6 rounded-full flex items-center justify-end pr-2" style="width: <?= max($percentage, 5) ?>%;">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Recandidaturas</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($topVigilantes as $index => $vigilante): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm font-medium text-gray-800"><?= htmlspecialchars($vigilante['name']) ?></td>
                                <td class="px-4 py-3 text-sm text-gray-600"><?= htmlspecialchars($vigilante['email']) ?></td>
                                <td class="px-4 py-3 text-sm text-center font-semibold text-gray-800"><?= $vigilante['total_applications'] ?></td>
                                <td class="px-4 py-3 text-sm text-center text-green-600"><?= $vigilante['approved_count'] ?></td>
                                <td class="px-4 py-3 text-sm text-center text-red-600"><?= $vigilante['rejected_count'] ?></td>
                                <td class="px-4 py-3 text-sm text-center text-gray-600"><?= $vigilante['total_reapplies'] ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="space-y-3">
                    <?php foreach ($topRejectionReasons as $reason): ?>
                        <div class="flex items-center justify-between p-3 bg-red-50 border border-red-100 rounded">
                            <div class="flex-1">
                                <p class="text-sm text-gray-800"><?= htmlspecialchars(substr($reason['rejection_reason'], 0, 60)) ?><?= strlen($reason['rejection_reason']) > 60 ? '...' : '' ?></p>
                            </div>
                            <div class="ml-3">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div class="mt-4 p-3 bg-gray-50 rounded">
            <p class="text-sm text-gray-600">
                ...
                <span class="text-lg font-bold text-green-600"><?= number_format($emailStats['success_rate'] ?? 0, 1) ?>%</span>
            </p>
        </div>
    </div>
</div>
