    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                ?>
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border <?= $statusClass ?>">
                ...
                <div class="space-y-6">
                    <?php foreach ($history as $index => $entry): ?>
                        ...
                        <div class="relative pl-12">
                            <!-- Ícone -->
                            <div
                                class="absolute left-0 w-8 h-8 rounded-full border-2 flex items-center justify-center <?= $newStatusColor ?>">
                                    ...
                                    <div>
                                        <?php if ($entry['old_status']): ?>
                                            <p class="text-sm text-gray-600">
                                                <span
                                                    class="font-medium"><?= htmlspecialchars(ucfirst($entry['old_status'])) ?></span>
                                                <span class="mx-2">→</span>
                                                <span
                                                    class="font-semibold text-gray-800"><?= htmlspecialchars(ucfirst($entry['new_status'])) ?></span>
                                            </p>
                                        <?php else: ?>
                                    ...
                                    <?php if ($metadata): ?>
                                        <div class="mt-2 pt-2 border-t border-gray-200">
                                            <details class="text-xs">
                                                <summary class="cursor-pointer text-gray-500 hover:text-gray-700">Detalhes técnicos
                                                </summary>
                                                <pre
                                                    class="mt-2 p-2 bg-gray-100 rounded text-xs overflow-x-auto"><?= htmlspecialchars(json_encode($metadata, JSON_PRETTY_PRINT)) ?></pre>
                                            </details>
                                        </div>
                                    <?php endif; ?>
                                <?php endif; ?>
