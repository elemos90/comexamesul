...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <?php if (isset($selectedVacancy)): ?>
                ...
                <button type="button" id="btn-approve-selected" data-disabled="true"
                ...
                <button type="button" id="btn-reject-selected" data-disabled="true"
    ...
    <div class="bg-white border border-gray-100 rounded-lg shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <form method="GET" action="<?= url('/applications') ?>" class="flex gap-3">
                <select name="vacancy" onchange="this.form.submit()"
                    ...
                    <?php foreach ($vacancies as $vacancy): ?>
                        <option value="<?= $vacancy['id'] ?>" <?= $selectedVacancy && $selectedVacancy['id'] == $vacancy['id'] ? 'selected' : '' ?>>
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="filter-search" placeholder="Nome ou email..."
                ...
                <div class="mt-3 text-xs text-gray-600">
                    Mostrando <span id="filtered-count" class="font-semibold">0</span> de <span id="total-count"
                        class="font-semibold">0</span> candidaturas
                </div>
            </div>
        <?php endif; ?>
    </div>
            ...
            <?php else: ?>
                <div class="divide-y divide-gray-100">
                    <?php foreach ($applications as $app): ?>
                        ...
                        ?>
                        <div data-application-row data-id="<?= $app['id'] ?>" data-status="<?= $app['status'] ?>"
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="pt-1">
                                            <input type="checkbox"
                                                ...
                                                data-app-status="<?= $app['status'] ?>" onchange="updateBulkButtons()"
                                        ...
                                        <div class="flex-1">
                                            <h3 class="font-semibold text-gray-800"><?= htmlspecialchars($app['vigilante_name']) ?>
                                            </h3>
                                            <div class="mt-1 flex flex-wrap items-center gap-3 text-sm text-gray-600">
                                                ...
                                                <span class="flex items-center gap-1">
                                                    ...
                                                    Candidatou-se:
                                                    <?= htmlspecialchars(date('d/m/Y H:i', strtotime($app['applied_at']))) ?>
                                                </span>
                                                <?php if ($app['supervisor_eligible']): ?>
                                                    <span
                                                        class="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs font-semibold rounded">
...
<div id="modal-reject"
    class="modal hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <form id="form-reject" class="p-6">
            ...
            <div class="flex gap-3 justify-end">
                ...
                <button type="submit" id="btn-submit-reject"
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 transform transition-all">
        <div class="p-6">
            <div id="confirm-icon-container"
                class="flex items-center justify-center w-12 h-12 mx-auto rounded-full mb-4 bg-green-100">
                <svg id="confirm-icon" class="w-6 h-6 text-green-600" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
            ...
            <p id="confirm-message" class="text-sm text-gray-600 text-center mb-6">Tem certeza?</p>
...
<script>
    ...
    function showConfirmModal(title, message, onConfirm, type = 'success') {
        ...
        iconContainer.className = `flex items-center justify-center w-12 h-12 mx-auto rounded-full mb-4 ${style.bg}`;
        iconContainer.querySelector('svg').className = `w-6 h-6 ${style.icon}`;
        btnOk.className = `flex-1 px-4 py-2.5 ${style.btn} text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2`;
    ...
    async function approveApplication(appId, vigilanteName) {
        ...
        <span class="text-green-600">✓ O candidato será aprovado para esta vaga.</span><br>
        <span class="text-sm text-gray-500">Um email de confirmação será enviado.</span>`,
            () => executeApprove(appId, vigilanteName),
            'success'
        );
    }

    async function executeApprove(appId, vigilanteName) {
        try {
            const response = await fetch(`${baseUrl}/applications/${appId}/approve`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('form-reject')?.addEventListener('submit', async function (e) {
        ...
        try {
            const response = await fetch(`${baseUrl}/applications/${appId}/reject`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
