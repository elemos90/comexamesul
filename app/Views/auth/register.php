...
<section class="py-4 md:py-8 bg-gray-50 min-h-screen flex items-center overflow-y-auto">
    <div class="max-w-2xl mx-auto bg-white shadow rounded-lg p-4 md:p-6 w-full my-4">
        ...
        <div class="flex justify-center mb-3 md:mb-4">
            <img src="/assets/images/logo_unilicungo.png" alt="UniLicungo - Logo" class="h-12 md:h-16 w-auto object-contain">
        </div>
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    <h3 class="text-sm font-semibold text-amber-900">
                        <?= $openVacanciesCount ?> Vaga<?= $openVacanciesCount > 1 ? 's' : '' ?> Aberta<?= $openVacanciesCount > 1 ? 's' : '' ?> para Vigilância
                    </h3>
        ...
        <form method="POST" action="/register" class="grid md:grid-cols-2 gap-3 md:gap-4">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
            <div class="md:col-span-2">
                <label for="name" class="block text-sm font-medium text-gray-700">Nome completo</label>
                <input type="text" id="name" name="name" value="<?= htmlspecialchars(old('name')) ?>" class="mt-1 w-full rounded border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500" required>
            ...
            <div>
                ...
                <input type="password" id="password" name="password" class="mt-1 w-full rounded border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500" required>
            ...
            <div>
                ...
                <input type="password" id="password_confirmation" name="password_confirmation" class="mt-1 w-full rounded border border-gray-300 px-3 py-2 focus:ring-primary-500 focus:border-primary-500" required>
            </div>
            <div class="md:col-span-2 flex items-center justify-between text-sm text-gray-600">
                ...
                <a href="/login" class="text-primary-600 font-medium">Ja tenho conta</a>
            </div>
            <div class="md:col-span-2">
                <button type="submit" class="w-full bg-primary-600 text-white font-semibold py-2 rounded hover:bg-primary-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">Criar conta</button>
                <p class="mt-3 text-xs text-gray-500">Depois de criar a conta, pode iniciar sessao com as suas credenciais.</p>
            </div>
        </form>
    </div>
</section>