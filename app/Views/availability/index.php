...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="mt-2 text-sm text-gray-600">Gerencie suas candidaturas às vagas abertas para participar como vigilante nos exames de admissão</p>
    </div>
        ...
        <div class="bg-red-50 border-2 border-red-200 rounded-lg p-6">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <a href="/profile" class="inline-flex items-center gap-2 px-5 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors">
        ...
        <div class="bg-white border border-gray-100 rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-3">
                <?php foreach ($myApplications as $app): ?>
                    ...
                    ?>
                    <div class="border border-gray-200 rounded-lg p-4 <?= $app['status'] === 'aprovada' ? 'bg-green-50' : '' ?>">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <?php if ($app['vacancy_status'] === 'fechada' || $app['vacancy_status'] === 'encerrada'): ?>
                                    ...
                                    ?>
                                    <span class="px-3 py-1.5 text-xs font-medium <?= $lockColor ?> rounded cursor-not-allowed" title="<?= $lockMsg ?>">
                                ...
                                <?php else: ?>
                                    <?php if ($app['status'] === 'pendente'): ?>
                                        ...
                                        <form method="POST" action="/applications/<?= $app['id'] ?>/cancel-direct" class="inline" onsubmit="return confirm('Deseja realmente cancelar esta candidatura?');">
                                            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                                            <button type="submit" class="px-3 py-1.5 text-xs font-medium bg-gray-100 text-gray-700 rounded hover:bg-gray-200 transition-colors" title="Cancelar candidatura">
                                                Cancelar
                                            </button>
                                        </form>
                                    <?php endif; ?>
                                    ...
                                    <?php if ($app['status'] === 'aprovada'): ?>
                                        ...
                                        <a href="/availability/<?= $app['id'] ?>/cancel" class="px-3 py-1.5 text-xs font-medium bg-red-100 text-red-700 rounded hover:bg-red-200 transition-colors" title="Solicitar cancelamento">
                                            Cancelar
                                        </a>
                                    <?php endif; ?>
                                    ...
                                    <?php if (in_array($app['status'], ['cancelada', 'rejeitada'])): ?>
                                        <!-- Recandidatar-se -->
                                        <form method="POST" action="/applications/<?= $app['id'] ?>/reapply" class="inline" onsubmit="return confirm('Deseja recandidatar-se a esta vaga?');">
                                            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                                            <button type="submit" class="px-3 py-1.5 text-xs font-medium bg-primary-600 text-white rounded hover:bg-primary-700 transition-colors" title="Recandidatar-se">
                                                Recandidatar-se
                                            </button>
                                        </form>
                                    <?php endif; ?>
                    ...
                    <div class="border-2 <?= $cardClass ?> rounded-lg p-5">
                        <div class="flex items-start justify-between gap-3 mb-3">
                            ...
                            <?php if ($daysLeft <= 3 && !$expired): ?>
                                <span class="flex-shrink-0 px-2 py-1 bg-amber-100 text-amber-700 text-xs font-semibold rounded-full">
                                    <?= $daysLeft ?> dia(s)
                                </span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="mb-4 text-sm text-gray-600 space-y-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span>Prazo: <strong><?= htmlspecialchars(date('d/m/Y H:i', strtotime($vacancy['deadline_at']))) ?></strong></span>
                            </div>
                        </div>
                        ...
                        <?php if ($myApplication): ?>
                            ...
                            <?php elseif ($myApplication['status'] === 'pendente'): ?>
                                ...
                                <?php if ($vacancy['status'] === 'aberta'): ?>
                                    <form method="POST" action="/applications/<?= $myApplication['id'] ?>/cancel-direct" class="mt-3" onsubmit="return confirm('Deseja cancelar esta candidatura?');">
                                        <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                            ...
                            <?php elseif ($myApplication['status'] === 'rejeitada'): ?>
                                <div class="space-y-3">
                                    ...
                                    <?php if (!$expired && $vacancy['status'] === 'aberta'): ?>
                                        <form method="POST" action="/applications/<?= $myApplication['id'] ?>/reapply" onsubmit="return confirm('Deseja recandidatar-se a esta vaga?');">
                                            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                                            <button type="submit" class="w-full px-3 py-2 text-sm font-medium bg-primary-600 text-white rounded hover:bg-primary-700 transition-colors">
                                                Recandidatar-me
                                            </button>
                                        </form>
                                    <?php else: ?>
                        ...
                        <?php elseif (!$profileComplete): ?>
                            <button disabled class="w-full px-4 py-2 bg-gray-300 text-gray-600 font-medium rounded cursor-not-allowed" title="Complete seu perfil para se candidatar">
                        ...
                        <?php else: ?>
                            <form method="POST" action="/vacancies/<?= $vacancy['id'] ?>/apply">
                                <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
                                <button type="submit" class="w-full px-4 py-2 bg-primary-600 text-white font-semibold rounded hover:bg-primary-700 transition-colors">
                                    <div class="flex items-center justify-center gap-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
