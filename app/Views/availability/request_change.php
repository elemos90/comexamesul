...
<div class="space-y-6">
    ...
    <div class="max-w-4xl">
        <div class="bg-white border border-gray-100 rounded-lg shadow-sm p-6">
            <div class="flex items-start gap-4 mb-6">
                ...
                <div class="flex-1">
                    <h1 class="text-2xl font-bold text-gray-800 mb-2">
                        Alterar Disponibilidade para <span class="text-<?= $statusColor ?>-600"><?= $statusText ?></span>
                    </h1>
                    <p class="text-sm text-gray-600">
                        Você está alocado a júris de exame. Para alterar sua disponibilidade, é necessário fornecer uma justificativa.
                    </p>
                </div>
            </div>
                    ...
                    <?php foreach ($allocations as $allocation): ?>
                        <div class="bg-white border border-<?= $statusColor ?>-200 rounded p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <p class="font-semibold text-gray-800"><?= htmlspecialchars($allocation['subject']) ?></p>
                                    <div class="mt-1 flex flex-wrap items-center gap-3 text-sm text-gray-600">
                                        ...
                                        <span class="flex items-center gap-1">
                                            ...
                                            <?= htmlspecialchars(substr($allocation['start_time'], 0, 5)) ?> - <?= htmlspecialchars(substr($allocation['end_time'], 0, 5)) ?>
                                        </span>
                                        <span class="flex items-center gap-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                                            </svg>
                                            <?= htmlspecialchars($allocation['location']) ?> - Sala <?= htmlspecialchars($allocation['room']) ?>
                                        </span>
                                    </div>
                                </div>
                            </div>
            ...
            <!-- Formulário -->
            <form method="POST" action="/availability/change/submit" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        placeholder="Descreva detalhadamente o motivo da alteração de disponibilidade (mínimo 20 caracteres)..."
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary-400 transition-colors">
                        ...
                        <label for="attachment" class="cursor-pointer">
                            ...
                            <p class="text-sm text-gray-600 mb-1"><span class="font-semibold text-primary-600">Clique para selecionar</span> ou arraste um arquivo</p>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-<?= $statusColor ?>-600 text-white font-semibold rounded-lg hover:bg-<?= $statusColor ?>-700 transition-colors shadow-sm">
                        Enviar Solicitação
                    </button>
                </div>
            </form>
