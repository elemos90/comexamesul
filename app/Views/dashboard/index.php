...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div class="bg-white border border-gray-200 rounded-lg p-3 md:p-4">
            ...
            <p class="text-xl md:text-2xl font-bold text-gray-900 mt-1"><?= (int) ($openVacancies ?? 0) ?></p>
        </div>
        ...
        <?php else: ?>
            <div class="bg-white border border-gray-200 rounded-lg p-3 md:p-4">
                ...
                <p class="text-xl md:text-2xl font-bold text-gray-900 mt-1"><?= (int) ($availableVigilantes ?? 0) ?></p>
            </div>
        <?php endif; ?>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-3 md:p-4">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <span
                    class="text-xl md:text-2xl font-bold text-gray-900"><?= isset($upcomingJuries) ? count($upcomingJuries) : 0 ?></span>
    ...
    <?php if ($isVigilante): ?>
        <!-- Alert Compacto -->
        <div class="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-md px-3 py-2 text-sm text-blue-700">
            <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="leading-tight">Atualize a sua <a href="<?= url('/availability') ?>"
                    class="underline font-medium">disponibilidade</a>
    ...
    <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <?php if (count($upcomingJuries) > 3): ?>
                <span class="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded self-start sm:self-center">
                    <?= count($upcomingJuries) ?> no mesmo dia
                </span>
            <?php endif; ?>
        </div>

        <div class="table-responsive max-h-[300px] overflow-y-auto">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php foreach ($upcomingJuries as $jury): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-3 py-2 text-gray-900 font-medium whitespace-normal sm:whitespace-nowrap">
                                <?= htmlspecialchars($jury['subject']) ?></td>
                            <td class="px-3 py-2 text-gray-600 whitespace-nowrap">
                                <?= date('d/m', strtotime($jury['exam_date'])) ?></td>
                            <td class="px-3 py-2 text-gray-600 whitespace-nowrap">
                                <?= substr($jury['start_time'], 0, 5) ?>-<?= substr($jury['end_time'], 0, 5) ?>
                            </td>
                            <td class="px-3 py-2 text-gray-600"
                                title="<?= htmlspecialchars($jury['location'] . ' / ' . $jury['room']) ?>">
                                <div class="truncate max-w-[100px] sm:max-w-[150px]">
