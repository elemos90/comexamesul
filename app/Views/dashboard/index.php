...
<div class="space-y-6">
    ...
    <div class="grid md:grid-cols-3 gap-5">
        ...
        <div class="bg-white border border-gray-100 rounded-lg p-5 shadow-sm">
            ...
            <p class="text-3xl font-semibold text-primary-600 mt-2">
                <?= isset($upcomingJuries) ? count($upcomingJuries) : 0 ?></p>
    ...
    <?php if ($isVigilante): ?>
        <div class="bg-blue-50 border border-blue-100 text-blue-800 text-sm rounded-lg px-4 py-3">
            Actualize a sua <a href="<?= url('/availability') ?>" class="underline font-medium">disponibilidade</a> sempre
            que necessitar. A comissao usa esta informacao para planear as vigias.
        </div>
    <?php endif; ?>
            ...
            <div>
                ...
                <?php if (!empty($upcomingJuries)): ?>
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        <?= count($upcomingJuries) ?> júri<?= count($upcomingJuries) > 1 ? 's' : '' ?>
                ...
                <thead class="bg-gray-50 sticky top-0 z-10">
                    <tr>
                        <th scope="col"
                            ...
                            Disciplina</th>
                        <th scope="col"
                            class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Data
                        </th>
                        <th scope="col"
                            class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Horario</th>
                        <th scope="col"
                            class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Local
                        </th>
                        <th scope="col"
                            class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Supervisor</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <?php foreach ($upcomingJuries as $jury): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600"><?= htmlspecialchars($jury['location']) ?> / Sala
                                <?= htmlspecialchars($jury['room']) ?></td>
                            <td class="px-4 py-3 text-sm text-gray-600">
                                <?= htmlspecialchars($jury['supervisor_name'] ?? '-') ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (!$upcomingJuries): ?>
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500">Sem registos a apresentar.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
