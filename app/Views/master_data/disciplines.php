                        ...
                        <?php foreach ($disciplines as $d): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="text-sm font-mono font-semibold text-gray-900"><?= htmlspecialchars($d['code']) ?></span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                    <button type="button"
                                        onclick="editDiscipline(<?= $d['id'] ?>, '<?= htmlspecialchars($d['code'], ENT_QUOTES) ?>', '<?= htmlspecialchars($d['name'], ENT_QUOTES) ?>', '<?= htmlspecialchars($d['description'] ?? '', ENT_QUOTES) ?>')"
                                    ...
                                    <?php if ((int) $d['jury_count'] === 0): ?>
                                        <form method="POST" action="<?= url('/master-data/disciplines/' . $d['id'] . '/delete') ?>"
                                            class="inline"
                                            onsubmit="return confirm('Eliminar esta disciplina? Esta ação não pode ser desfeita.');">
...
<div class="modal fixed inset-0 hidden items-center justify-center z-50" id="modal-create-discipline" role="dialog"
    aria-hidden="true">
    ...
    <div class="modal-content relative bg-white w-full max-w-lg mx-4 rounded-lg shadow-lg p-6" tabindex="-1">
        ...
        <form method="POST" action="<?= url('/master-data/disciplines') ?>" class="space-y-4">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
            <div>
                ...
                <input type="text" id="discipline_code" name="code" maxlength="20"
                    class="mt-1 w-full rounded border border-gray-300 px-3 py-2 uppercase" placeholder="Ex: MAT1, FIS1"
                    required>
                <p class="mt-1 text-xs text-gray-500">Código único para identificação (será convertido em maiúsculas)
                </p>
            </div>
            <div>
                ...
                <input type="text" id="discipline_name" name="name" maxlength="180"
                    class="mt-1 w-full rounded border border-gray-300 px-3 py-2" placeholder="Ex: Matemática I"
                    required>
            </div>
            <div>
...
<div class="modal fixed inset-0 hidden items-center justify-center z-50" id="modal-edit-discipline" role="dialog"
    aria-hidden="true">
    <div class="modal-backdrop absolute inset-0 bg-gray-900/50"></div>
    <div class="modal-content relative bg-white w-full max-w-lg mx-4 rounded-lg shadow-lg p-6" tabindex="-1">
        <div class="flex items-center justify-between mb-4">
        ...
        <form method="POST" id="form-edit-discipline" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700"
                    for="edit_discipline_description">Descrição</label>
