...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border p-4">
        <form method="GET" action="<?= url('/master-data/rooms') ?>" class="flex items-end gap-4">
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-2" for="location">Selecionar Local *</label>
                <select id="location" name="location" class="w-full rounded border border-gray-300 px-3 py-2"
            ...
            <button type="submit"
                class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-500">
                Filtrar
            </button>
        </form>
    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">
                            <?= htmlspecialchars($selectedLocation['code']) ?> -
                            <?= htmlspecialchars($selectedLocation['name']) ?>
                        </h2>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Edifício/Piso</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Júris
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Status</th>
                            ...
                            <?php foreach ($rooms as $r): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="text-sm font-mono font-semibold text-gray-900"><?= htmlspecialchars($r['code']) ?></span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <button type="button"
                                            onclick='editRoom(<?= json_encode($r, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                                        ...
                                        <?php if ((int) ($r['jury_count'] ?? 0) === 0): ?>
                                            <form method="POST" action="<?= url('/master-data/rooms/' . $r['id'] . '/delete') ?>"
                                                class="inline"
                                                onsubmit="return confirm('Eliminar esta sala? Esta ação não pode ser desfeita.');">
...
<!-- Modal: Criar Sala -->
<div class="modal fixed inset-0 hidden items-center justify-center z-50" id="modal-create-room" role="dialog"
    aria-hidden="true">
    ...
    <div class="modal-content relative bg-white w-full max-w-lg mx-4 rounded-lg shadow-lg p-6" tabindex="-1">
        ...
        <form method="POST" action="<?= url('/master-data/rooms') ?>" class="space-y-4">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="room_code" name="code" maxlength="20"
                        class="mt-1 w-full rounded border border-gray-300 px-3 py-2 uppercase" placeholder="Ex: A101"
                        required>
                </div>
                <div>
                    ...
                    <input type="number" id="room_capacity" name="capacity" min="1"
                        class="mt-1 w-full rounded border border-gray-300 px-3 py-2" placeholder="Ex: 50" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="room_building" name="building" maxlength="60"
                        class="mt-1 w-full rounded border border-gray-300 px-3 py-2" placeholder="Ex: Bloco A">
                </div>
                <div>
                    ...
                    <input type="text" id="room_floor" name="floor" maxlength="30"
                        class="mt-1 w-full rounded border border-gray-300 px-3 py-2" placeholder="Ex: 1º Andar">
                </div>
            </div>
...
<!-- Modal: Editar Sala -->
<div class="modal fixed inset-0 hidden items-center justify-center z-50" id="modal-edit-room" role="dialog"
    aria-hidden="true">
    <div class="modal-backdrop absolute inset-0 bg-gray-900/50"></div>
    <div class="modal-content relative bg-white w-full max-w-lg mx-4 rounded-lg shadow-lg p-6" tabindex="-1">
        <div class="flex items-center justify-between mb-4">
